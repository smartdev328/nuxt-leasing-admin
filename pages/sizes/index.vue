        ...
        <b-card :header="header">
          ...
          >
            <template slot="id" slot-scope="data">
              <router-link :to="`/sizes/${data.item.id}`">
                {{ data.item.id }}
              </router-link>
            </template>
  ...
  methods: {
    getSizes(pageNum) {
      ...
      axios.get('/api/v1/sizes/', {
        ...
        this.sizes = response.data.results
        ...
        this.header = `List of All Sizes (${startIndex} - ${endIndex} / ${this.totalRows})`
      })
    },
    deleteSize(sizeId) {
      this.loading = true
