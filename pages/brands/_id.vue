<template>
  <div class="brands">
    <b-row>
      <b-col lg="12">
        <h2>Edit Brand - {{ formData.name }}</h2>
      </b-col>
      <b-col lg="12">
    ...
    <b-row class="justify-content-between align-items-center actions-group">
      ...
      <div>
        <b-button type="submit" variant="primary" @click="updateBrand()">
          <i class="fa fa-dot-circle-o" /> Save
        </b-button>
        &nbsp;&nbsp;
        <b-button type="button" variant="danger" @click="deleteBrand()">
...
export default {
  ...
  mounted() {
    ...
    this.loading = true
    axios.get(`/api/v1/brands/${this.brandId}`).then(response => {
  ...
  methods: {
    updateBrand() {
      ...
      if (valid) {
        this.resetValidate()
        this.loading = true
        axios.put(`/api/v1/brands/${this.brandId}`, {
          ...this.formData
        })
          .then(response => {
            this.loading = false
    ...
    deleteBrand() {
      this.loading = true
      axios.delete(`/api/v1/brands/${this.brandId}`)
        .then(response => {
          this.loading = false
